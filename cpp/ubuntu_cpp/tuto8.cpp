...
void drawTriangle()
{
    ...
    glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 8 * sizeof(float), (void *)0); //LAYOUT POSITION
    glEnableVertexAttribArray(0);
    glVertexAttribPointer(1, 3, GL_FLOAT, GL_FALSE, 8 * sizeof(float), (void *)(3 * sizeof(float))); //LAYOUT COLOR
    ...
    do
    {
        ...
        if (glfwGetKey(window, GLFW_KEY_W) == GLFW_PRESS && y <= 5.6)
        {
            printf("up: %f\n", y);
            y += 0.01;
        }
        if (glfwGetKey(window, GLFW_KEY_S) == GLFW_PRESS && y >= -1)
        {
            printf("Down: %f\n", y);

            y -= 0.01;
        }
        if (glfwGetKey(window, GLFW_KEY_D) == GLFW_PRESS && x <= 1)
        {
            printf("right: %f\n", x);
            x += 0.01;
        }
        if (glfwGetKey(window, GLFW_KEY_A) == GLFW_PRESS && x >= -1)
        {
            printf("left: %f\n", x);
